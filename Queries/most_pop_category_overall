...
WITH categorised_meals AS (
  SELECT
    CASE 
      ...
      WHEN meal ILIKE '%fried%' OR meal ILIKE '%steak%' OR meal ILIKE '%chicken%' 
        OR meal ILIKE '%sausage%' OR meal ILIKE '%shrimp%' OR meal ILIKE '%fish%' THEN 'Main Meal'
      WHEN meal ILIKE '%oatmeal%' OR meal ILIKE '%eggs%' THEN 'Breakfast Item'
      WHEN meal ILIKE '%coffee%' OR meal ILIKE '%milk%' OR meal ILIKE '%tea%' 
        OR meal ILIKE '%water%' OR meal ILIKE '%coke%' OR meal ILIKE '%dr pepper%' THEN 'Drink'
...
-- Step 2: Count number of requests per category and calculate pct of total
SELECT 
  meal_category,
  COUNT(*) AS total_requests,
  ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM death_row_meal), 1) AS pct_of_total
